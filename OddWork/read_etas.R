...
etas_data <- dplyr::filter(data, data$Quantity == c("eta.fu"))

# Re-organises etas_data into panel data format
etas_data <- reshape2::melt(etas_data, 
...
names(etas_data)[names(etas_data) == 'variable'] <- 'Year'

# Re-names column "value" to "eta"
names(etas_data)[names(etas_data) == 'value'] <- 'eta'


# Writes a .csv file for the etas_data DF
write.csv(etas_data, file = "C:/Github/PFU-Database/OddWork/etas_data.csv")
